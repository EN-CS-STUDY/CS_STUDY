...
- 소켓 함수는 리턴하지 않고 대기상태에 들어간다
- 멀티스레드를 사용해 다른 작업을 하는 것이 아니라면 애플리케이션은 더 이상 진행되지 않는다
- **socket()은 기본적으로 블로킹 소켓이다**(1:1)
...
<img width="600" alt="image" src="https://github.com/EN-CS-STUDY/CS_STUDY/assets/100523178/2487e1db-49b2-4cd7-9441-424a5d01be1e">

> 두 개념은 표현 형태는 비슷해 보이지만, 서로 다른 차원에서 작업의 수행 방식을 설명한다<br>
>
> `동기/비동기`
> - **완료 시점**을 어떻게 처리하는가
> - 요청 작업에 대한 `완료 여부`가 중요하다
> - 작업을 순차적으로 진행할지 아닌지에 대한 관점이다
>   
> `블로킹/논블로킹`
> - 작업의 **수행 여부**를 어떻게 결정하는가
> - 호출된 함수가 호출한 함수에게 `제어권`을 바로 주는지로 구분한다
